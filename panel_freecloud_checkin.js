...
// 流程: Puppeteer 浏览器 → 登录页 → 填邮箱/密码 → 登录 → 每日签到
...
// Chromium 路径：优先用 Playwright 安装的，回退到 puppeteer 默认
let CHROMIUM_PATH;
try {
    CHROMIUM_PATH = require('playwright').chromium.executablePath();
} catch (e) {
...
async function attemptTurnstileClick(page) {
    ...
    // 方式2: 通过 __turnstile_data 坐标 + mouse.click
    try {
        const frames = page.frames();
        for (const frame of frames) {
            const fu = frame.url() || '';
            ...
            if (data) {
                ...
                if (!box) continue;
                const clickX = box.x + (box.w * data.xRatio);
...
async function waitCfChallengeDone(page) {
    for (let i = 0; i < 40; i++) {
        ...
        const isCf = /security|challenge|verify|正在验证|安全检查|please wait/i.test(title)
...
async function passCloudflare(page, readySelector, label) {
    for (let i = 0; i < 8; i++) {
        const ready = await isVisible(page, readySelector);
        if (ready) {
...
async function startSocks5HttpProxy(socksUrl) {
    ...
    server.on('connect', (req, clientSocket, head) => {
        ...
        SocksClient.createConnection({
            proxy: { host: proxyHost, port: proxyPort, type: 5, userId: proxyUsername, password: proxyPassword },
    ...
    try {
        ...
        if (!loggedIn) {
            ...
            if (!cfReady) {
                ...
                const pageBody = await getBodyText(page);
            ...
            if (currentUrl.includes('/login')) {
                throw new Error('登录失败 — 仍停留在登录页，导航结果: ' + navResult);
        ...
        const icon = success ? '✅' : '❌';
        await sendTelegramMessage(icon + ' *FreeCloud 签到* — ' + userIdentifier + '\n' + msg, shotPath);
        ...
        try { await page.screenshot({ path: shotPath, fullPage: true }); } catch (e) { }
        await sendTelegramMessage('❌ *FreeCloud 签到失败* — ' + userIdentifier + '\n错误: ' + err.message, shotPath);
        throw err;
    } finally {
        await browser.close();
