...
# -*- coding: utf-8 -*-
"""
XServer Game 免费服务器自动续期 — 青龙面板调用版
通过 GitHub API 触发 xserver.yml workflow_dispatch
"""
...
WORKFLOW_FILE = os.environ.get('WORKFLOW_FILE', 'xserver.yml')
...
def trigger_workflow_dispatch(token):
    ...
    headers = {
        ...
        'User-Agent': 'qinglong-xserver/1.0'
    }
    payload = {
        'ref': WORKFLOW_REF
    }
...
def main():
    print('=' * 55)
    print('  XServer Game 服务器自动续期 — 青龙触发版')
    ...
    if not token:
        ...
        print('  WORKFLOW_FILE  - workflow 文件名 (默认: xserver.yml)')
